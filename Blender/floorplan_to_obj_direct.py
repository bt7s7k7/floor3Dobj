...
"""
Floorplan to OBJ Direct Export

FloorplanToBlender3d
Copyright (C) 2021 Daniel Westberg

This code read data from a file and creates a 3d model of that data, 
then exports it directly to OBJ format instead of saving as .blend first.
RUN THIS CODE FROM BLENDER

This is a modified version that exports directly to .obj format to improve workflow.
"""
...
def main(argv):
    ...
    """
    Export directly to OBJ format
    Instead of saving as .blend first, we export directly to .obj
    """
    # Extract the base path without extension and add .obj
    obj_path = target
    if target.endswith('.blend'):
        obj_path = target[:-6] + '.obj'  # Replace .blend with .obj
    elif not target.endswith('.obj'):
        obj_path = target + '.obj'
    
    # Export to OBJ format (compatible with Blender 4.x)
    try:
        # Try new Blender 4.x method
        bpy.ops.wm.obj_export(filepath=program_path + obj_path)
    except AttributeError:
        try:
            # Try older method
            bpy.ops.export_scene.obj(filepath=program_path + obj_path)
        except AttributeError:
            # Fallback - save as blend and convert later
            bpy.ops.wm.save_as_mainfile(filepath=program_path + obj_path.replace('.obj', '.blend'))
            print("Warning: Could not export to OBJ directly, saved as .blend file instead")
